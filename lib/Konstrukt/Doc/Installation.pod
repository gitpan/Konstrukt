...
This is a (complete?) list of 3rd-party perl modules used in this framework.
...
Basically the configuration for an apache vhost would look like this:
...
in your config. In Debian, you would just enable the mod by creating a symlink
/etc/apache2/mods-enabled/apreq.load -> /etc/apache2/mods-available/apreq.load.
...
auto-initialization mechanism, which is turned off by default for performance
reasons. The settings are documented in L<Konstrukt::Handler/CONFIGURATION>.
You may turn it on before the first use, try out all pages/plugins and turn it
off again. You may also want to take a look at the supplied
C<KonstruktBackendInitialization.pl> script, which manually initializes all
backends that you need. You should call this script as the same user as your
webserver runs as.
...
The served files are processed by different handlers, whose configuration is
